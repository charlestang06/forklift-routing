...
def load_aggregate_data(
    ...
    try:
        ...
        print("Failed to read cleaned data")
...
def get_user_tasks(df: pd.DataFrame, user: str) -> pd.DataFrame:
    ...
    """
    if user in get_list_users(df):
        df = df[df["user"] == user]
        return df
    else:
        print("User not found in df")
        return None


def get_shipping_doors_volume(dc: int = 800) -> pd.DataFrame:
    """
    Returns list of list corresponding shipping door with its volume of LPNs

    Inputs: dc (int)
    Outputs: shipping_doors_counts ([[30, 160], [140, 2000], ...])
    """
    ...
    if dc == 820:
        dock_doors = set(
            [i for i in range(23, 47)]
            + [i for i in range(48, 89, 2)]
            + [i for i in range(106, 139, 2)]
            + [i for i in range(141, 176)]
        )
        dock_doors_to_remove = set([60])
        dock_doors = sorted(list(dock_doors - dock_doors_to_remove))
    if dc == 840:
        dock_doors = set(
            [i for i in range(155, 174, 2)]
            + [i for i in range(106, 175, 2)]
            + [i for i in range(55, 88, 2)]
            + [i for i in range(56, 89, 2)]
        )
