...
class RoutingModel:
    ...
    def strip_locn_name(name: str):
        if "_" in name:
            return name[: name.index("_")]
        return name

    def calcuate_distance_tasks(self, tasks: List[List]):
        distance = sum(
            [
...
class HistoricalData(RoutingModel):
    ...
    def __init__(self, distance_map: dict, tasks: List[List]):
        ...
        for i in range(self.forklifts):
            ...
            while j < tasks_user.shape[0]:
                ...
                if j < tasks_user.shape[0] - 1:
                    ...
                    if task == next_task:
                        # add a new task from end of this task to beginning of after skipped task so # pallets calculation stays same
    ...
    def assign_tasks(self) -> dict:
        ...
        if self.forklift_starting_locns:
            ...
            for i in range(self.forklifts):
                ...
                task_done_times[i] = self.time_from_distance(
                    closest_distance
...
class RandomGroupsTSP(RoutingModel):
    ...
    def assign_tasks(self) -> dict:
        ...
        for key in self.forklift_tasks.keys():
            tasks = self.forklift_tasks[key]
...
class VRP(RoutingModel):
    """
    Use OR-Tools for VRPPD. Assigns each task as a "city".
    Goal: Minimize total distance. Assume only one pallet at a time.
...
class ShippingLaneModel:
    ...
    def __init__(
        ...
        self.top_k = top_k
        self.calibration_date = calibrationDate
    ...
    def calculate_distance_map(self) -> dict:
        ...
        shipping_doors_set = set([str(x) for x in self.old_new_door_map.keys()])
...
class ClusterModel(ShippingLaneModel):
    ...
    def assign_doors(self) -> dict:
        ...
        for i in range(len(new_door_order)):
            # check if False
            if self.top_k and new_door_order[i] != False:
                self.old_new_door_map[new_door_order[i]] = self.shipping_doors[i]
                self.old_new_door_map[self.shipping_doors[i]] = new_door_order[i]
            elif not self.top_k:
                self.old_new_door_map[new_door_order[i]] = self.shipping_doors[i]
...
class GreedyModel(ShippingLaneModel):
    ...
    def assign_doors(self) -> dict:
        ...
        i = 0
        df = load_aggregate_data(dc=self.dc, date=self.calibration_date)
        dist_map = self.original_distance_map
        # only data in this week
        df = df[df["from_time"].between("2024-05-22", "2024-05-29", inclusive="both")]
                    ...
                    if x[(t, g)].value() == 1:
                        if self.top_k:
                            if self.shipping_doors[t] in top_k_shipping_doors:
                                ...
                                for i in range(len(self.shipping_doors_counts)):
                                    if (
                                        self.shipping_doors_counts[i][0]
                                        == self.shipping_doors[g]
                                    ):
                                        top_k_shipping_doors_counts.append(
                                            self.shipping_doors_counts[i]
                                        )
                                        break
